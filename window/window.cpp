...
MenuTab currentTab = MenuTab::ESP;

void RenderESP()
{
	ImGui::Text("ESP");
	ImGui::Checkbox("Enable ESP", &ConfigMenu::showEsp);
	ImGui::Checkbox("Enable Bones", &ConfigMenu::showBones);
	ImGui::Checkbox("Enable Traacers", &ConfigMenu::showTracers);
	ImGui::Checkbox("Enable Filled Box", &ConfigMenu::filledBox);
	ImGui::Checkbox("Show ped", &ConfigMenu::showPeds);

	// فقط اگه Tracer روشن بود، Dropdown نشون بده
	if (ConfigMenu::showTracers)
	{
		const char* positions[] = { "Top", "Center", "Bottom" };
		int selected = static_cast<int>(ConfigMenu::tracerStartPosition);
		if (ImGui::Combo("Tracer Start", &selected, positions, IM_ARRAYSIZE(positions)))
		{
			ConfigMenu::tracerStartPosition = static_cast<ConfigMenu::TracerStartPosition>(selected);
		}
	}
}
...
{
	ImGui::Text("Visuals");
	ImGui::Checkbox("Enable Croshair", &ConfigMenu::showCroshair);
}
void Overlay::Render()
{
	...
	ImGui::BeginGroup();
	{
		if (ImGui::Button("ESP", ImVec2(150, 0)))
			currentTab = MenuTab::ESP;

		if (ImGui::Button("Visuals", ImVec2(150, 0)))
			currentTab = MenuTab::Visuals;

		if (ImGui::Button("Self", ImVec2(150, 0)))
			currentTab = MenuTab::Self;
	}
	ImGui::EndGroup();

	// Rysowanie aktywnego taba
	ImGui::Separator();
	switch (currentTab)
	{
	case MenuTab::ESP:
